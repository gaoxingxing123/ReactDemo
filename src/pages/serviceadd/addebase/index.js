...
class Base extends Component {
  ...
  saveChange(data){    
    let url='http://192.168.3.236:8088/CorporateInfo/'+this.props.form.getFieldValue('clientName')
    axios.get(url    
    ).then((res)=>{
  ...
  render() {
    ...
    const options = this.state.list.map(d => <Option key={d.id}>{d.clientName}</Option>);
        return (
            <MyFormContent > 
            ...
            <div style={{float:'left',width:'40%',marginLeft:'8%'}}>                 
                ...
                <Button style={{marginRight:'23%',marginLeft:'20%'}} onClick={ ()=>this.saveChange(this.state.data)}
                  ...
                  <Button> <Link to={'/addeloan/'+data.id}>下一步</Link></Button> 
                  <Button>                    
                      <Link to={'/'}>返回</Link>
                  </Button>
